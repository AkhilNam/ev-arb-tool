...
import { fetchNcaabOdds } from '../api';
...
const NcaabResults = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await fetchNcaabOdds();
        setOdds(data);
        setLoading(false);
      } catch (err) {
        setError('Failed to fetch NCAAB odds data');
        setLoading(false);
      }
    };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">NCAAB EV Opportunities</h1>
          <p className="text-muted-foreground">Real-time college basketball betting opportunities</p>
        </div>
